...
for j = 1:numJ0
    ...
    parfor i = 1:num_all
        ...
        if vv >= 1e-3
            ...
            if nonlinear_id == 3.5
                [LD_FTLE_i,LD_a2_i,Np_prl_i,KEarth_i,KMoon_i,KEscape_i,tend,xend,eve] = ...
                    get_Dy_LD_cons_tend_mex(t0,t_end,num_tend,y0,...
                        dyn_str,LD_str,eps,exp_alp,keta);
...
%%
    for kt = 1:num_tend
        ...
        if N_valid > 0
            TC_probility(j,kt) = numel(Np_prl_all_real(valid_id & Np_prl_all_real >= 1))/N_valid;
            BC_probility_t(j,kt) = numel(Np_prl_all_real(valid_id & Np_prl_all_real >= 2))/N_valid;
        end
        for ithr = 1:num_thr
    ...
    save(strcat(filepat1,'\','LD_',num2str(nonlinear_id),'_',LD_str,'_expalp',num2str(exp_alp),...
            ...
            'Np_prl_all_t','r2','v_vert','dyn_str','eps','exp_alp','dx_eps','LD_str','z0','dz0',...
            'KMoon_all_t','KEarth_all_t','KEscape_all_t','LDa2_thr','stat_TC','stat_BC','metric_name',...
...
save(strcat(filepat,'LD_threshold_changeT_summary_',datestr(datetime('today'),'mmdd'),'.mat'),...
