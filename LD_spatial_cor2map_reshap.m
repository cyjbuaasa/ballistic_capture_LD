...
for j = 1:1
    ...
    BC_map = containers.Map('KeyType', 'char', 'ValueType', 'any');
    ...
    if exist(filepat2,'dir')~=0 %%判断文件夹是否存在
        disp('dir is exist,continue \n');
        %%如果文件夹存在，输出:dir is exist
        load(strcat(filepat2,'\LD_3.5_H2_expalp1.25_0414.mat'),...
            'LD_FTLE_mesh','LD_a2_mesh','Np_prl_all');
        LD_BC = LD_FTLE_mesh;LD_BC(Np_prl_all<2)=nan;
        str_BC = struct('LD',LD_FTLE_mesh,'LDsec',LD_a2_mesh,...
            'LD_BC',LD_BC,'Np',Np_prl_all);
        BC_map(key) = str_BC;
        continue;
    end
    ...
    BC_map(key) = struct('LD',LD_FTLE_mesh,'LDsec',LD_a2_mesh,'LD_BC',LD_BC,'Np',Np_prl_all);
    %% LD
...
save_name_base = strcat('LD_BCmap_',num2str(nonlinear_id),'_',LD_str,'_expalp',num2str(exp_alp),...
...
save(save_path,'BC_map','BC_spa_probility',...
    't0','t_end','nonlinear_id','J0','alp_mesh','psi1_mesh','-v7.3');
end
toc;


%% 得到更简洁的只包含TC 和 BC 的数据,确实内存更小
...
for k = 1:length(psi2)
    ...
    key = strcat('gama',num2str(gama(n)/rad),'_psi2',num2str(psi2(k)/rad));
    tmp=BC_map(key);
    if size(tmp.LD,1) == 721
        alp = [0:0.5:360]*rad;psi1 = [0:0.5:360]*rad;
    else
        alp = [0:1:360]*rad;psi1 = [0:1:360]*rad;
    end
    [row_TC, col_TC] = find(tmp.LDsec > 1.0);
    ind_TC = sub2ind(size(tmp.LD), row_TC, col_TC);
    TC_ang = [alp(col_TC);psi1(row_TC);]';
    LD_TC = tmp.LD(ind_TC);

    [row_BC, col_BC] = find(tmp.Np>=2);
    BC_ang = [alp(col_BC);psi1(row_BC);]';
    ind_BC = sub2ind(size(tmp.LD), row_BC, col_BC);
    LD_BC = tmp.LD(ind_BC);

    BC_mapnew(key) = struct('LD_TC',LD_TC,'TC_ang',TC_ang,'LD_BC',LD_BC,'BC_ang',BC_ang,...
        'Np',tmp.Np);
    end
end

% LD_BCmapnew_ 数据量更少
save_name_base2 = strcat('LD_BCmapnew_',num2str(nonlinear_id),'_',LD_str,'_expalp',num2str(exp_alp),...
    '_',datestr(datetime('today'),'mmdd'));
save_name = strcat(save_name_base2,'.mat');
save_path = fullfile(filepat1,save_name);
if exist(save_path,'file') == 2
    save_name = strcat(save_name_base,'.mat');
    save_path = fullfile(filepat1,save_name);
...
for k = 1:length(psi2)
    ...
    LD_FTLE_mesh = tmp.LD;LD_a2_mesh = tmp.LDsec;Np_prl_all = tmp.Np;
    LD_FTLE_mesh(LD_FTLE_mesh == 0) = nan;% 有效 FTLE map
    LD_a2_mesh(LD_a2_mesh == 0) = nan; % 有效a2 LD map
    Np_prl_all(isnan(LD_a2_mesh)) = nan;
    Np_prl_all((LD_a2_mesh <= 1.0)) = nan;% 有效近月点map
...
%% plot 正确做法不再需要太多数据
key = strcat('gama',num2str(gama(10)/rad),'_psi2',num2str(psi2(10)/rad));
tmp=BC_mapnew(key);
...
fig_LD = figure('Position', [46.60,192.2,949.60,560], 'Color', 'white');
 ...
 set(gca,'FontName','Times New Roman','FontSize',25,'FontWeight','normal');
 set(LD_field,'ButtonDownFcn',{@mycall_map_spatial,hAx,...
         t0,t0+t_end,alp_mesh,psi1_mesh,BC_map(key).LD_BC,r2,gama(10),psi2(10),...
         J0(j),nonlinear_id,m_STT,dyn_str,eps,exp_alp,dx_eps,LD_str});
