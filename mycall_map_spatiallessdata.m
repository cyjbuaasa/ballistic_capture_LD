function mycall_map_spatiallessdata(hObject,~,hAx,t0,t_end,alp_mesh,psi1_mesh,orbDEX,...
    r2,gama,psi2,J0,nonlinear_id,m_STT,...
    ...
    [v, idx] = min((alp_mesh(:)-x).^2 + (psi1_mesh(:)-y).^2);
    [row, col] = ind2sub(size(alp_mesh), idx);
    fprintf(' alp: %f\n psi: %f\n', alp_mesh(row, col)/rad, psi1_mesh(row, col)/rad);
    fprintf(' orbDex = %f\n ',orbDEX(row, col));
    % 小于3像素点再绘图防止误点击
    mu = 0.0121506683;
    ome_sun = -0.925195985;
    ...
    if v < 3
        r2_vec = r2*[cos(alp_mesh(row, col))*cos(gama);...
            sin(alp_mesh(row, col))*cos(gama);sin(gama)];
        r1_vec = [1;0;0]+r2_vec;
        r1 = norm(r1_vec);
        ...
        vv = fsolve(@(v)(1*(v^2)-((1-mu+r2_vec(1))^2+r2_vec(2)^2) ...
            ...
            1e-2,option_LL);
        T = [cos(gama),0,sin(gama);0,1,0;-sin(gama),0,cos(gama)]*...
            [cos(alp_mesh(row, col)),sin(alp_mesh(row, col)),0;...
            -sin(alp_mesh(row, col)),cos(alp_mesh(row, col)),0;0,0,1];
        v_vlocal = vv*[sin(psi2);cos(psi2)*cos(psi1_mesh(row, col));...
            cos(psi2)*sin(psi1_mesh(row, col))];
        v0 = T\v_vlocal;
        ...
        [orb_dynam_index,Np_prl,eve_struct,t,X,tend,xend,eve] = get_Dyindex(t0,t_end*2.3,y0,nonlinear_id,m_STT,...
            dyn_str,eps,alp,dx_eps,LD_str,options_LD,mu);
        % alpha=0:0.01:2*pi;
