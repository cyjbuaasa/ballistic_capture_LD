function [value,isterminal,direction] = only_hit_LD(t,X,~,~,~,~,~)
%%% 防撞击检测
...
% 分别检测：1 撞月、2 撞地、3 到达逃逸边界、4 近地点、5 远地点、6 近月点/7 离轨时刻
    ...
    if norm(X(1:3)-Rm) <= 1.50*66180/DU && Esec <= 0
        value = [norm(R2)-((1738)/DU);norm(R1)-((6378)/DU);radial_velocity_m;...
            norm(R2)-(6*66180/DU);];% norm(R2)-(6*66180/DU)
        isterminal = [1;1;0;0;]; 
        direction = [0;0;1;1;];   % 
        % 3:perilune
    else
        value = [norm(R2)-((1738)/DU);norm(R1)-((6378)/DU);1;...
            norm(R2)-(6*66180/DU);];
        isterminal = [1;1;0;0;]; 
        direction = [0;0;1;1;];   % 
        % 3:perilune
